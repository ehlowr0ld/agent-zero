...
class SchedulerTaskUpdate(ApiHandler):
    async def process(self, input: Input, request: Request) -> Output:
        ...
        if "ctx_planning" in input:
            update_params["ctx_planning"] = input.get("ctx_planning", "")

        if "ctx_reasoning" in input:
            update_params["ctx_reasoning"] = input.get("ctx_reasoning", "")

        if "ctx_deep_search" in input:
            update_params["ctx_deep_search"] = input.get("ctx_deep_search", "")

...
class AdHocTask(BaseModel):
    ...
    ctx_planning: Literal["on", "off", "auto"] = Field(default="auto")
    ctx_reasoning: Literal["on", "off", "auto"] = Field(default="auto")
    ctx_deep_search: Literal["on", "off"] = Field(default="off")
    ...
    def create(
        ...
        prompt: str,
        token: str,
        ctx_planning: Literal["on", "off", "auto"] = "auto",
        ctx_reasoning: Literal["on", "off", "auto"] = "auto",
        ctx_deep_search: Literal["on", "off"] = "off",
        ...
        return cls(name=name,
                   ...
                   token=token,
                   ctx_planning=ctx_planning,
                   ctx_reasoning=ctx_reasoning,
                   ctx_deep_search=ctx_deep_search)
    ...
    def update(self,
               ...
               last_run: datetime | None = None,
               last_result: str | None = None,
               ctx_planning: Literal["on", "off", "auto"] | None = None,
               ctx_reasoning: Literal["on", "off", "auto"] | None = None,
               ctx_deep_search: Literal["on", "off"] | None = None,
               token: str | None = None):
        with self._lock:
            if name is not None:
                self.name = name
            ...
            if last_result is not None:
                ...
                self.updated_at = datetime.now(timezone.utc)
            if ctx_planning is not None:
                self.ctx_planning = ctx_planning
                self.updated_at = datetime.now(timezone.utc)
            if ctx_reasoning is not None:
                self.ctx_reasoning = ctx_reasoning
                self.updated_at = datetime.now(timezone.utc)
            if ctx_deep_search is not None:
                self.ctx_deep_search = ctx_deep_search
                self.updated_at = datetime.now(timezone.utc)
...
class TaskScheduler:
    ...
    async def __new_context(self, task: Union[ScheduledTask, AdHocTask]) -> AgentContext:
        ...
        chat_names.set_name(context.id, task.name)
    ...
    async def _run_task(self, task: Union[ScheduledTask, AdHocTask]):
        ...
        async def _run_task_wrapper(task_uuid: str):
            ...
            try:
                ...
                # We clear these at the beginning of a run,
                # because we don't want to carry over any state from previous runs
                # This also enables us to see tasklist and notepad from a previous run
                agent.tasklist.clear()
                agent.notepad.clear()
                        ...
                        if os.path.exists(attachment):
                            ...
                            try:
                                ...
                                if url.scheme in ["http", "https", "ftp", "ftps", "sftp"]:
                                    attachment_filenames.append(url.netloc + url.path)
                                else:
                ...
                context.log.log(
                    ...
                    kvps={"attachments": current_task.attachments},
                    id=str(uuid.uuid4()),
                )

                system_message = []
                if context.reasoning == "on":
                    system_message_reasoning = agent.parse_prompt(
                        "fw.msg_system_use_reasoning.md"
                    )
                    system_message.append(system_message_reasoning)
                if context.planning == "on":
                    system_message_planning = agent.parse_prompt(
...
def serialize_task(task: Union[ScheduledTask, AdHocTask]) -> Dict[str, Any]:
    ...
    task_dict = {
        ...
        "ctx_deep_search": task.ctx_deep_search,
...
def deserialize_task(task_data: Dict[str, Any], task_class: Optional[Type[T]] = None) -> T:
    ...
    common_args = {
        ...
        "ctx_planning": task_data.get("ctx_planning", "auto"),
        "ctx_reasoning": task_data.get("ctx_reasoning", "auto"),
        "ctx_deep_search": task_data.get("ctx_deep_search", "off"),

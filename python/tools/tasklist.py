...
# TODO: add message to the task log when changing status
...
class TaskListTool(Tool):
    ...
    def _task_status_filter(self, args: dict[str, str]) -> list[TaskStatus]:
        ...
        if isinstance(status, str):
            status_filter: list[TaskStatus] = [TaskStatus(status)]
        elif isinstance(status, list):
            status_filter: list[TaskStatus] = [TaskStatus(s) for s in list(status)]
        else:
            message = self.agent.read_prompt(
